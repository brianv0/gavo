...
class TableInterface(StandardQueryMixin):
	...
	def getIndices(self):
		"""returns a mapping of index names to fields taking part in them.

		The special index name "/" is always ignored.  It can be used to
		tell introspecting code that there is an index on a field but that
		the index must not be generated by TableWriter (typically, because
		it is generated by a script).
		"""
		indices = {}
		for field in self.fields:
			if field.get_index() and field.get_index()!='/':
