...
sys.path.insert(0, "..")
import setup as toplevel_setup

PKGDIR = "pkgdir"
# Set to have sdist copy the tarballs in debian-compatible naming to this dir
TARBALL_DIR = os.path.expanduser("~/gavo/tarballs")

setupHead = """# setup.py for GAVO DC subpackage.
# Automatically generated by subpkgs/subsetup.py in the DC main source
# tree.  Edit that file rather than this one.
...
baseKeys = dict((key, toplevel_setup.SETUP_ARGS[key])
	for key in "url license author author_email version".split())
baseKeys["namespace_packages"] = ["gavo"]
...
subpkgs = {
	...
	"votable": {
		...
		"install_requires": ["gavoutils"],
		"dependency_links": ["http://vo.ari.uni-heidelberg.de/soft/dist"],
		"MAIN_TREE_FILES": ["gavo/votable", "docs/votable.rstx", 
			"docs/tapquery.rstx"],
		"DEBIAN_NAME": "python-gavovotable",
	},
}
...
def copyTarballUp(debianName, packageDef):
	if debianName and TARBALL_DIR:
		srcPath = os.path.join(os.getcwd(), "dist", "%s-%s.tar.gz"%(
			packageDef["name"], packageDef["version"]))
		destPath = os.path.join(TARBALL_DIR, "%s_%s.orig.tar.gz"%(
			debianName, packageDef["version"]))
...
def procOne(subName, setupArgs):
	...
	amendManifest(pkgDir,
		copyTreeFiles(pkgDir, packageDef.pop("MAIN_TREE_FILES", [])))
	debianName = packageDef.pop("DEBIAN_NAME", None)
	fullDesc = makeSetup(pkgDir, **packageDef)
	oDir = os.getcwd()
	os.chdir(pkgDir)
	retcode = subprocess.call(["python", "setup.py"]+setupArgs)
	if retcode==0 and setupArgs and setupArgs[0]=="sdist":
		copyTarballUp(debianName, fullDesc)
	os.chdir(oDir)
