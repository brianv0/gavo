"""
Generate a schemaLocations.property file as used by XSD validation tests.
"""
...
class TNSIs(Exception):
	def __init__(self, tns):
		self.tns = tns
		Exception.__init__(self, "TNS found")


class TNSExtractor(ContentHandler):
	def startElement(self, name, attrs):
		if attrs.has_key("targetNamespace"):
			raise TNSIs(attrs["targetNamespace"])

_extractor = TNSExtractor()

def extractTargetNamespace(fName):
	try:
		xml.sax.parse(open(fName), _extractor)
	except TNSIs, tnsEx:
		return tnsEx.tns
	return None
...
def getLocations():
	locations = []
	for fName in glob.glob("*.xsd"):
		tns = extractTargetNamespace(fName)
		if tns:
			locations.append((tns, "file://"+os.path.abspath(fName)))
	return locations
...
public class xsdval
    ...
    public static void main(String argv[]) {
        ...
        for (int i = 0; i < argv.length; i++) {
            ...
            try {
              parser.setProperty("http://apache.org/xml/properties/schema/external-schemaLocation", "%s");
            // parse file
            } catch ( SAXException e) {
